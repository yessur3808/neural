...
import cn.ms.neural.moduler.engine.faulttolerant.entity.HystrixSetter;
...
public class HystrixSetterSupport {
	...
	public static HystrixCommand.Setter buildSetter(HystrixSetter hs) {
    	...
    	setter=setter.andCommandPropertiesDefaults(HystrixCommandProperties.Setter()
    			//超时控制
    			.withExecutionIsolationThreadTimeoutInMilliseconds(hs.getExecutionIsolationThreadTimeoutInMilliseconds())//执行隔离线程超时毫秒,默认为1000ms
    			.withExecutionTimeoutInMilliseconds(hs.getExecutionTimeoutInMilliseconds())//执行超时时间,默认为1000ms
    			//断路器
    			.withCircuitBreakerRequestVolumeThreshold(hs.getCircuitBreakerRequestVolumeThreshold())//当在配置时间窗口内达到此数量的失败后,进行短路,默认20个
    			.withCircuitBreakerSleepWindowInMilliseconds(hs.getCircuitBreakerSleepWindowInMilliseconds())//短路多久以后开始尝试是否恢复,默认5s
    			.withCircuitBreakerErrorThresholdPercentage(hs.getCircuitBreakerErrorThresholdPercentage())//出错百分比阈值,当达到此阈值后,开始短路,默认50%
    			//调用线程允许请求HystrixCommand.GetFallback()的最大数量，默认10。超出时将会有异常抛出，注意：该项配置对于THREAD隔离模式也起作用
    			.withFallbackIsolationSemaphoreMaxConcurrentRequests(hs.getFallbackIsolationSemaphoreMaxConcurrentRequests()));
    	...
    	setter=setter.andThreadPoolPropertiesDefaults(HystrixThreadPoolProperties.Setter()
    			...
    			//Queue
    			.withQueueSizeRejectionThreshold(hs.getThreadPoolQueueSizeRejectionThreshold()));//排队线程数量阈值，默认为5，达到时拒绝，如果配置了该选项，队列的大小是该队列
